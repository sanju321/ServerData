...
def get_firmware_token():
    ...
    f = open("data_system_4_to_23_02_2015.csv",'w')
    ...
    strurl="https://kito.azoi.com//api/test-jig/v1/data/?from_ts="+start_date_url+"&to_ts="+end_date_url+"&filter=_id"#for main server
    ...
    rJson1 = r1.json()
    print "rJson1------------------------------------------------------------------------", rJson1
    dct = rJson1['results']['testjig_data']
    print "dct---------------------------------------------------------------------------",dct
    ...
    first = True
    new_list=[]
    fw=open("./system_server_data_4_to_23_02_2015.txt","w")
    for d in dct:
        try:
            cnt+=1
            ...
            f.flush()
            print "row ",cnt,"   ",data['results']['testjig_data'],"sid  ",sid
            fw.write("row "+str(cnt)+"   "+str(data['results']['testjig_data'])+"sid  "+str(sid)+"\n")
        except:
            new_list.append(sid)
    ...
    for n in new_list:
        try:
            cnt+=1
            sid = n
            strurl="https://kito.azoi.com//api/test-jig/v1/data/?testjig_data_id="+str(sid)#for main server
